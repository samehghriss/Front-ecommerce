...
const Checkout = ({ products, setRun = (f) => f, run = undefined }) => {
    ...
    const showDropIn = () => ( <div onBlur = {
        ...
        { " " } {
            data.clientToken !== null && products.length > 0 ? ( <div >
                ...
                <button class = "btn btn-secondary" > Redeem </button>{" "}</div>{" "} </div>{" "} { /* </form> */ } { " " } <
                div className = "mt-3" >
                <
                label
                for = "email"
    ...
    return ( <div >
        < h2 > Total: Dt{ getTotal() } </h2>{" "} <p> Card Number: "4111 1111 1111 1111" </p> {showLoading(data.loading)}{" "} { showSuccess(data.success) } { showError(data.error) } { showCheckout() } { " " } </div>
    );
};
